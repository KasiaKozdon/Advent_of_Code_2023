...
def expand_galaxy(galaxies_map, expansion_steps=1):
    """
    :param galaxies_map:
    :param expansion_steps: for part 2 of the puzzles
    ...
    for idx, row in enumerate(galaxies_map):
        if row == len(row) * row[0]:  # check if all characters are the same
            expanded_input[idx + n_added_rows:idx + n_added_rows] = [row for _ in range(expansion_steps)]
            n_added_rows += expansion_steps
    for idx, column in enumerate(list(zip(*expanded_input))):
        if column == len(column) * (column[0],):
            expansion_insertion = "."*expansion_steps
            expanded_input = [f"{line[:idx+n_added_columns]}{expansion_insertion}{line[idx+n_added_columns:]}"
                              for line in expanded_input]
            n_added_columns += expansion_steps
    return expanded_input
...
def solve_puzzle_part2(galaxies_map):
    galaxies_map = galaxies_map.strip().split("\n")
    expanded = expand_galaxy(galaxies_map, expansion_steps=1000000)
    galaxy_coords = find_all_galaxies(expanded)
    galaxy_pairs = pair_galaxies(galaxy_coords)
    return sum(find_shortest_distance(galaxy_pairs))
